<?php
 ...
 GalleryHeader();
  ...
  $db->setQuery('SELECT count(*) FROM #__datsogallery_purchases WHERE user_id = '.$user->id);
  $total = $db->loadResult();
  $pageNav = new JPagination($total, $limitstart, $limit);
  $page_nav_links = $pageNav->getPagesLinks();
  $db->setQuery('SELECT a.*, p.image_id, p.date'
  .' FROM #__datsogallery AS a,'
  .' #__datsogallery_catg AS c,'
  .' #__datsogallery_purchases AS p'
  .' WHERE a.catid = c.cid'
  .' AND a.published = 1'
  .' AND a.id = p.image_id'
  ...
  $pw_title = JText::_('COM_DATSOGALLERY_MY_PURCHASES');
   ...
   if ($ad_showdetail) {
    ...
    <span>
              <?php
               ...
               echo '<br />';
               canDownload ($row->id, $row->catid, $row->imgprice);
?>
    </span>
            <?php
             echo '</div>';
