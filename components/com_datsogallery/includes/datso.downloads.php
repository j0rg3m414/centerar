<?php
 ...
 GalleryHeader();
  ...
  $db->setQuery('SELECT count(*) FROM #__datsogallery WHERE imgdownloaded > 0 AND published = 1 AND approved = 1');
  $total = $db->loadResult();
  $pageNav = new JPagination($total, $limitstart, $limit);
  $page_nav_links = $pageNav->getPagesLinks();
  $db->setQuery('SELECT *, imgdownloaded AS download'
  .' FROM #__datsogallery AS a,'
  .' #__datsogallery_catg AS ca'
  .' WHERE a.catid = ca.cid'
  .' AND a.imgdownloaded > 0'
  .' AND a.published = 1'
  .' AND a.approved = 1'
  .' AND ca.approved = 1'
  .' AND ca.published = 1'
  ...
  $pw_title = JText::_('COM_DATSOGALLERY_MOST_DOWNLOADED');
   ...
   if ($ad_showdetail) {
    ...
    <span>
              <?php
               ...
               echo ($ad_showfrating) ? showVote ($row->id, $row->imgvotes, $row->imgvotesum) : '';
               if ($ad_showcomments) {
