...
.' AND c.published = 1'
.' AND c.access IN ('.$groups.')'
.' ORDER BY a.imgcounter DESC'
;
$db->setQuery($query, $pagination->limitstart, $pagination->limit);
$rows = $db->loadObjectList();
$pw_title = JText::_('COM_DATSOGALLERY_MOST_VIEWED');
...
if (count($rows) > 0) {
  foreach ($rows as $row) {
    ...
    $ld = "<a href='".JRoute::_("index.php?option=com_datsogallery&amp;task=image&amp;catid="
    .$row->catid."&amp;id=".$row->id.$itemid)."' title='".$row->imgtitle."'>";
    echo $ld;
